...
class AdaptSelfAttention(nn.Module):
    def __init__(self, hidden_size, num_heads,
        ...
        # self.i_d=nn.Linear(self.hidden_size,1)###
        ...
        if dataset == 'boson':
            self.randomAttention = nn.Parameter(torch.empty(1, self.num_heads, 600, 600), requires_grad=True)
        if dataset == 'peopledaily':
            self.randomAttention = nn.Parameter(torch.empty(1, self.num_heads, 1000, 1000), requires_grad=True)
        nn.init.kaiming_normal_(self.randomAttention, a=math.sqrt(5))

    def forward(self, query, key, value, seq_len,pos_ss,pos_ee, lex_num, rel_pos_embedding):

        # v_i = self.i_d(torch.tanh(self.w_d(key)))#####
        ...
        # s_l = torch.unsqueeze(seq_len,dim=-1)#####
        # s_l = torch.unsqueeze(s_l, dim=-1)#####
        # D_i = torch.matmul(v_i,s_l.to(torch.float32))#####
        ...
        # GS
        pos_d = -abs(pow((pos_ss + pos_ee) / 2, 2)) / (2 * pow(1, 2))
        pos_d = pos_d.to('cuda')
        pos_d = (1 / (pow(2 * math.pi, 0.5) * 1)) * torch.exp(pos_d)
        GS=pos_d
        GS2=torch.unsqueeze(torch.tile(GS[0], (self.num_heads, 1, 1)),0)
        for i in range(1,batch):
            GS1 = torch.tile(GS[i], (self.num_heads, 1, 1))
            GS1 = torch.unsqueeze(GS1,0)
            GS2 = torch.cat([GS2,GS1],0)
        attn_score_raw =attn_score_raw+GS2
