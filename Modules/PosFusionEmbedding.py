...
class PosFusionEmbedding(nn.Module):
    ...
    def forward(self, pos_s, pos_e):
        ...
        pe_ss1 = self.pe_ss[(pos_ss).view(-1) + self.max_seq_len].view(size=[batch, max_seq_len, max_seq_len, -1])
        pe_ee1 = self.pe_ee[(pos_ee).view(-1) + self.max_seq_len].view(size=[batch, max_seq_len, max_seq_len, -1])

        pe_2 = torch.cat([pe_ss1, pe_ee1], dim=-1)
        rel_pos_embedding = self.pos_fusion_forward(pe_2)

        return rel_pos_embedding,pos_ss,pos_ee
