...
@cache_results(_cache_fp='cache/boson_ner', _refresh=False)
def load_boson_ner(path, char_embedding_path=None, bigram_embedding_path=None, index_token=True,
                    char_min_freq=1, bigram_min_freq=1, only_train_min_freq=0):
    train_path = os.path.join(path, 'boson.train.bmes')
    dev_path = os.path.join(path, 'boson.dev.bmes')
    test_path = os.path.join(path, 'boson.test.bmes')
...
@cache_results(_cache_fp='cache/peopledaily_ner', _refresh=False)
def load_peopledaily_ner(path, char_embedding_path=None, bigram_embedding_path=None, index_token=True,
                    char_min_freq=1, bigram_min_freq=1, only_train_min_freq=0):
    train_path = os.path.join(path, 'train_new.bmes')
    dev_path = os.path.join(path, 'dev_new.bmes')
    test_path = os.path.join(path, 'tes_new.bmes')
...
def equip_chinese_ner_with_lexicon(datasets, vocabs, embeddings, w_list, word_embedding_path=None,
    ...
    def get_skip_path(chars, w_trie):
        ...
        return result
    def get_char_lex(chars,w_trie):
        sentence = ''.join(chars)
        lexicons = w_trie.get_lexicon(sentence)
        match=[]
        for i in range(len(chars)):
            match.append([])
            max_len = 0
            for j in range(len(lexicons)):
                if chars[i] in lexicons[j][2] and lexicons[j][1]-lexicons[j][0]+1>max_len:
                    max_len=lexicons[j][1]-lexicons[j][0]+1
            match[i].append(max_len)
        return match
    ...
    for k, v in datasets.items():
        ...
        v.apply_field(partial(get_char_lex, w_trie=w_trie), 'chars','char_lex_max_len')
