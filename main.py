...
from Modules.CNNzixingEmbedding import CNNzixingEmbedding
...
parser.add_argument('--batch', default=10, type=int)  #10
...
parser.add_argument('--dataset', default='resume', help='weibo|resume|ontonotes|msra|boson|peopledaily')
...
if args.dataset == 'ontonotes':
...
elif args.dataset == 'boson':
    datasets, vocabs, embeddings = load_boson_ner(boson_ner_path, yangjie_rich_pretrain_unigram_path,
                                                   yangjie_rich_pretrain_bigram_path,
                                                   _refresh=refresh_data, index_token=False,
                                                   _cache_fp=raw_dataset_cache_name,
                                                   char_min_freq=args.char_min_freq,
    ...
    datasets, vocabs, embeddings = load_peopledaily_ner(peopledaily_ner_path, yangjie_rich_pretrain_unigram_path,
                                                   yangjie_rich_pretrain_bigram_path,
                                                   _refresh=refresh_data, index_token=False,
                                                   _cache_fp=raw_dataset_cache_name,
                                                   char_min_freq=args.char_min_freq,
...
if args.dataset == 'resume':
    ...
    args.cj_wb_dropout = 0.5
    args.char_dropout = 0.5
    args.head_dim = 16
    args.ff = 384
    args.hidden = 128
    args.warmup = 0.05
    ...
    args.cj_wb_embed_lr_rate = 0.0007
    args.momentum = 0.9
    args.epoch = 50
    args.seed =9249
    ...
    args.cj_wb_embed_lr_rate = 0.0005
    args.momentum = 0.9
    args.update_every = 1
    args.epoch = 100
elif args.dataset == 'msra':
    ...
    args.cj_wb_embed_lr_rate = 0.0012
    args.momentum = 0.85
    args.update_every = 1
    args.epoch = 100
elif args.dataset == 'weibo':
    ...
    args.hidden = 128
    args.radical_dropout = 0.2
    args.cj_wb_dropout = 0.2
    args.warmup = 0.3
    args.lr = 0.0018
    args.components_embed_lr_rate = 0.0014
    args.cj_wb_embed_lr_rate = 0.0014
    args.momentum = 0.9
    args.epoch = 50
# elif args.dataset == 'boson':
#     args.ff_dropout = 0.1
...
datasets, vocabs, embeddings = equip_chinese_ner_with_lexicon(datasets, vocabs, embeddings,  #FLAT
...
embeddings['cj_wb'] = CNNzixingEmbedding(vocab=vocabs['lattice'], embed_size=50, char_emb_size=30,
                                                    filter_nums=[30],
                                                    kernel_sizes=[3], char_dropout=args.char_dropout,
                                                    dropout=args.cj_wb_dropout, pool_method='max'
                                                    , include_word_start_end=False, min_char_freq=1)

model = MECTNER(embeddings['lattice'], embeddings['bigram'],embeddings['cj_wb'], args.hidden,
...
cj_wb_embed_param = list(model.cj_wb_embed.parameters())
...
embedding_param_ids = list(map(id, embedding_param + cj_wb_embed_param))
...
param_ = [{'params': non_embedding_param},
          {'params': embedding_param, 'lr': args.lr * args.embed_lr_rate},
          {'params': cj_wb_embed_param, 'lr': args.cj_wb_embed_lr_rate}]
